...
export default function HeaderProfiles1({ navigation }) {
    ...
    const onClickAddImage = () => {
        ...
        ActionSheet.show(
            { options: BUTTONS, cancelButtonIndex: 2, title: 'Chọn hình ảnh' }, (btnSelect) => {
    ...
    const updateInfo = () => {
        navigation.navigate("FormStaff1", { title: "Cập nhật thông tin", type: "UPDATE", stafflist: user, fromInfo: "Profile" })
    }
        ...
        <View style={{ backgroundColor: '#FFF', flex: 1 }}>
            ...
            {type == 0 ?
                ...
                <ProfileButton text='Settings' icon='cogs' cb={signOut}/> */}
                    <ProfileButton text='Đăng xuất' icon='sign-out-alt' cb={signOut} />
                </View>) : type == 1 ?
                    (<View style={styles.boxapp}>
                        <ProfileButton text='Hóa đơn' icon='file-alt' cb={showOrderChef} />
                        {/* <ProfileButton text='Settings' icon='info' cb={signOut}/>
                <ProfileButton text='Settings' icon='cogs' cb={signOut}/> */}
                        <ProfileButton text='Đăng xuất' icon='sign-out-alt' cb={signOut}
                        />
                        ...
                        <Overlay visible={open} fullScreen={false} overlayStyle={{ height: '70%' }}>
                            <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                                <Text style={{ color: '#458', fontSize: 16, fontWeight: 'bold' }}>Danh sách hóa đơn</Text>
                    ...
                    (<>
                        <View style={styles.boxapp}>
                            <ProfileButton text='Danh sách món ăn' icon='file-alt' cb={showOrderChef} />
                            {/* <ProfileButton text='Settings' icon='info' cb={signOut}/> */}
                            <ProfileButton text='Báo cáo' icon='cogs' cb={showCookChef} />

                            <Overlay visible={reportPop} fullScreen={false} overlayStyle={{ height: '70%' }}>
                                <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <Text style={{ color: '#458', fontSize: 16, fontWeight: 'bold' }}>Danh sách món ăn</Text>
